...
* Framework: Qt 6 (Widgets + WebEngine), dynamically linked under GPL-3 (WebEngine requires dynamic linking due to Chromium licensing).
* Language: C++17/20.
* Database: SQLite3 (FTS5 enabled).
* Build System: CMake.
* Package format: Application executable + Qt runtime libraries per OS (standard Qt deployment model).

=== 2.5 Design and Implementation Constraints

* Must comply with GPL-3 license.
* All code written in C++17/20 using Qt 6 framework.
* Qt 6 dynamically linked (GPL-3 compatible, WebEngine requirement).
...
Codexium Magnus and its components are released under GPL-3. Qt 6 is dynamically linked under GPL-3 license (WebEngine requires dynamic linking due to Chromium licensing constraints).
