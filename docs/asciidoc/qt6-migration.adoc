...
|*Scope:*
|Complete migration path from Avalonia + CEF to Qt 6 (GPL-3, dynamically linked) while preserving all functional requirements and architectural principles.
|===

== 1. Executive Summary

This document outlines a *complete cutover migration* for Codexium Magnus from the current *Avalonia UI + CEF* stack to *Qt 6 (GPL-3, dynamically linked)*. The project is at PoC stage, enabling a "knife-edge cutover" approach. The migration preserves all functional requirements (FR-1 through FR-12), maintains the layered architecture, and ensures cross-platform compatibility across Windows, macOS, and Linux. Note: Qt WebEngine requires dynamic linking due to Chromium licensing constraints.

=== 1.1 Key Decisions

1. *Qt 6 GPL-3* with *dynamic linking* — required for Qt WebEngine (Chromium-based), fully compatible with GPL-3 license.
...
* *Build System*: .NET SDK → CMake + Qt build tools (dynamic linking - WebEngine requirement)
* *Testing*: Port test harnesses to Qt Test framework
* *Runtime*: Remove .NET dependency entirely

== 2. Licensing Analysis

=== 2.1 Qt 6 GPL-3 Dynamic Linking

**Conclusion**: Qt 6 must use *dynamic linking* due to Qt WebEngine's Chromium foundation, which has licensing constraints preventing static linking.

**Rationale**:

* Qt's GPL-3 license permits both static and dynamic linking when the application is GPL-3.
* However, **Qt WebEngine is based on Chromium**, which has licensing constraints that prevent static linking.
* Codexium Magnus requires Qt WebEngine for HTML rendering, so dynamic linking is mandatory.
* For consistency and to avoid mixed linking scenarios, all Qt components use dynamic linking.
* No licensing conflicts exist with dynamic linking under GPL-3.

**Requirements**:

* Use pre-built Qt 6 (GPL-3) with dynamic linking, or build from source with dynamic linking enabled.
* Distribute Qt runtime libraries with the application (standard Qt deployment model).
...
* Application executable + Qt runtime libraries (DLLs/.so/.dylib).
* Standard Qt deployment tools can be used (`windeployqt`, `macdeployqt`, `linuxdeployqt`).
* Qt runtime libraries must be included in distribution package.

=== 2.2 License Compliance Checklist

* [ ] Use Qt 6 (GPL-3) with dynamic linking (WebEngine requirement)
* [ ] Configure CMake for dynamic linking (`-DBUILD_SHARED_LIBS=ON` or default)
* [ ] Include Qt GPL-3 license text in distribution
* [ ] Include all Qt source code (or link to official Qt sources)
* [ ] Document Qt version and dynamic build configuration
* [ ] Ensure all dependencies are GPL-3 compatible
* [ ] Use Qt deployment tools to bundle runtime libraries
...
* Establish CMake build system with dynamic linking
* Create minimal Qt Widgets shell
* Integrate Qt WebEngine for HTML rendering
* Verify cross-platform dynamic build and deployment
...
* Platform-specific dynamic linking and deployment considerations
...
cd qtwebengine
# Follow Qt WebEngine build instructions (dynamic linking required)
----

**Option B: Use Pre-built GPL-3 Qt (Recommended)**

* Use Qt's official distribution channels (Qt Online Installer)
* Select GPL-3 license option
* WebEngine requires dynamic linking (standard Qt distribution)
* Use Qt deployment tools (`windeployqt`, `macdeployqt`, `linuxdeployqt`) to bundle runtime libraries

**Platform-Specific Notes**:

* *Windows*: Use `windeployqt` to bundle Qt DLLs with application
* *macOS*: Use `macdeployqt` to create .app bundle with Qt frameworks
* *Linux*: Use `linuxdeployqt` or manual library bundling; ensure all Qt dependencies included

**Verification**:

[source,bash]
----
# After installation, verify dynamic linking and WebEngine functionality
ldd /opt/qt6/bin/qmake  # Should show Qt library dependencies (.so files)
file /opt/qt6/bin/qmake  # Should indicate dynamic binary
# Verify WebEngine is available
/opt/qt6/bin/qmake -query QT_INSTALL_LIBS  # Check Qt library location
ls /opt/qt6/lib/libQt6WebEngine*  # Verify WebEngine libraries exist
----
...
**CMake Structure (Dynamic Linking)**:
...
# Dynamic linking configuration (WebEngine requires dynamic linking)
set(BUILD_SHARED_LIBS ON)  # Default, but explicit for clarity

# Find Qt 6 (must be built with dynamic linking for WebEngine)
...
# Qt WebEngine requires dynamic linking due to Chromium licensing
...
# Link dynamically (default for Qt)
...
1. **Install Qt 6 (GPL-3)**: Use pre-built Qt 6 with dynamic linking (WebEngine requires dynamic linking)
2. **POC Phase 1**: Build minimal Qt WebEngine app with dynamic linking to validate approach
3. **Team Training**: Schedule Qt 6 and CMake training sessions
4. **License Verification**: Confirm Qt GPL-3 dynamic linking compliance (already verified — proceed)
5. **Select Ed25519 Library**: Choose and integrate libsodium or alternative

=== 11.2 Decision Points

* *End of Phase 1*: Confirm Qt WebEngine dynamic build viability, decide Widgets vs. QML
...
* *Performance Optimization*: Profile and optimize C++/Qt code
* *Community*: Engage Qt community for support and best practices
* *Deployment Maintenance*: Document Qt dynamic linking deployment process for future updates
...
* Qt Deployment: https://doc.qt.io/qt-6/deployment.html[Qt Deployment] (dynamic linking)
...
* [x] CMake project structure created with dynamic linking configuration
...
|2025-11-11
|System
|Updated for GPL-3 dynamic linking (WebEngine requirement) and full C++/Qt migration (knife-edge cutover). Added styling strategy section (6.6).
